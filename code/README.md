...
- Typical command: not run directly.
- Checker: Conversion output plus tests around heuristic selection.
- Notes: The March 4, 2025 cutoff remains the current production behavior. A single T1w keeps the historical unsuffixed name; multiple T1w acquisitions are emitted as `run-1`, `run-2`, and so on. When `NORM` MPRAGE reconstructions exist they are preferred; when none exist, all exact full-MPRAGE protocol matches are retained. Low-resolution `t1_fl2d_sag_p2` series are not structural T1w inputs. Uses the same localizer/PhoenixZIPReport filename filter as `heuristics_rf1.py`.
