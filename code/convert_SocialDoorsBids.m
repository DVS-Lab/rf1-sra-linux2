...
% This script reads code/sublist-new.txt, searches each subject's behavioral
% data for available SocialDoors and Doors event files, and writes all files
% to the bids func
% directory, even if a run is behaviorally invalid. Behavioral validity
% is tracked separately through printed summaries and ignored log files.
...
% Outputs are BIDS-compatible event files under bids/ and behavioral-validity
% summaries under ignored logs/behavior-validity/.

this_file = mfilename('fullpath');
codedir = fileparts(this_file);
basedir = fileparts(codedir);
datadir = '/ZPOOL/data/projects/rf1-sra/stimuli/Scan-Social_Doors/data';
batch_subjects_file = fullfile(basedir, 'code', 'sublist-new.txt');

% Load the current batch subject list.
subject_lines = strtrim(readlines(batch_subjects_file));
subject_lines = subject_lines(subject_lines ~= "");
subject_lines = subject_lines(~startsWith(subject_lines, "#"));
subjects = cellstr(erase(subject_lines, "sub-"));
